$(function () {
    $("#dg").datagrid({
        rowStyler: function (index, row) {
            if (row.devResult == 0) {
                return "background-color:RGB(199,237,233)"
            } else if (row.devResult == 1) {
                return "background-color:RGB(227,160,93)"
            } else if (row.devResult == 2) {
                return "background-color:RGB(29,191,151)"
            }else {
...
function querySaleChancesByParams() {
    $("#dg").datagrid("load", {
        ...
        devResult: $("#devResult").datebox("getValue"),
